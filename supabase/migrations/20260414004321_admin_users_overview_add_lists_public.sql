-- Add lists_public column to admin_users_overview so the admin detail hero
-- can conditionally render a "View public collection" link only when the
-- user has made their collection public.
...
select
  ...
  coalesce(p.lists_public, false) as lists_public
from auth.users u
