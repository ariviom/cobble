...
import { generateRebrickableCsv } from "@/lib/export/rebrickableCsv";
...
export function ExportModal({ open, onClose, setNumber, setName, getMissingRows }: Props) {
	const [target, setTarget] = useState<"rebrickable" | "bricklink">("rebrickable");
	...
	function onExport() {
		...
		if (unmapped.length > 0) {
			setError(`${unmapped.length} rows could not be mapped to BrickLink colors and were skipped.`);
		}
