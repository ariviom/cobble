...
- **Local-first storage via Dexie/IndexedDB** (`app/lib/localDb/`) for catalog caching and owned quantities.
...
- **Local-first UX**: All users (anonymous and authenticated) store owned quantities and catalog cache in IndexedDB via Dexie. Authenticated users sync to Supabase via a background sync worker that batches writes through `/api/sync`. Supabase SSR is used only for auth-aware surfaces (layout/theme, account, user sets, pricing, group host actions), not for core catalog flows.
