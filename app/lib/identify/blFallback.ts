...
import { getSetsForPart, resolvePartIdToRebrickable } from '@/app/lib/rebrickable';
...
): Promise<{ sets: BLSet[]; subsets: Awaited<ReturnType<typeof blGetPartSubsets>> }> {
	let setsFromBL = toBLSet(await withBudget(budget, () => blGetPartSupersets(blId, initialColorId)));
	let subsets: Awaited<ReturnType<typeof blGetPartSubsets>> = [];
	if (setsFromBL.length >= supersetLimit) return { sets: setsFromBL.slice(0, supersetLimit), subsets };
...
async function loadCachedSupersets(
	blId: string
): Promise<{ sets: BLSet[]; partName: string | null; partImage: string | null; source: BLSource } | null> {
	try {
		...
		const source = (setRows[0]?.source as BLSource | undefined) ?? 'bl_supersets';
...
export async function fetchBLSupersetsFallback(
	...
	if (!setsFromBL.length && subsets.length) {
		...
		const components = subsets
			...
			.filter(c => typeof c.partNum === 'string') as { partNum: string; colorId?: number }[];
		...
		if (perComponentSets.length === limited.length && perComponentSets.length > 0) {
	...
	if (!setsFromBL.length && subsets.length) {
		for (const sub of subsets.slice(0, Math.min(subsets.length, colorVariantLimit))) {
	...
	if (!setsFromBL.length && subsets.length) {
		...
		if (setHitMap.size) {
			setsFromBL = Array.from(setHitMap.values())
				.sort((a, b) => b.hits - a.hits || b.set.quantity - a.set.quantity || b.set.year - a.set.year)
				.map(entry => entry.set)
	...
	try {
		...
		partImage = normalizeBLImageUrl(
			typeof metaWithImage.image_url === 'string' ? metaWithImage.image_url : partImage
		);
	} catch (err) {
		if (isBudgetError(err)) throw err;
	}
