...
export function extractCandidatePartNumbers(
  payload: BrickognizeResponse
): Array<{
  partNum: string;
  confidence: number;
  colorId?: number;
  ...
  if (Array.isArray(payloadWithArrays.candidates))
    arrays.push(...payloadWithArrays.candidates);
  if (Array.isArray(payloadWithArrays.results))
    arrays.push(...payloadWithArrays.results);
  if (Array.isArray(payloadWithArrays.matches))
    arrays.push(...payloadWithArrays.matches);
  // Legacy predict shape: items[]
  if (Array.isArray(payloadWithArrays.items))
    arrays.push(...payloadWithArrays.items);
  ...
  if (arrays.length) {
    for (const anyC of arrays) {
      ...
      const partNum =
        ...
        (typeof c.rebrickable_part_num === 'string' &&
          c.rebrickable_part_num) ||
        (typeof c.bricklink_part_num === 'string' && c.bricklink_part_num) ||
        (typeof c.bricklink_part_num === 'number' &&
          String(c.bricklink_part_num)) ||
        '';
      if (!partNum) continue;
      const confidence =
        typeof c.confidence === 'number'
          ...
          : typeof c.score === 'number'
            ...
            : 0;
      const colorId =
        (c.colorId as number) ?? (c.color_id as number) ?? undefined;
      const colorName =
        (c.colorName as string) ?? (c.color_name as string) ?? undefined;
      const imageUrl =
      ...
      if (blField) {
        ...
        const sites = c.external_sites as Array<{
          name?: string;
          url?: string;
        }>;
        ...
        const byName = normalized.find(
          s => s.name.replace(/\s+/g, '') === 'bricklink'
        );
        ...
        for (const s of candidatesForParse) {
          ...
          const isBrickLink =
            (byName && s === byName) ||
            (typeof url === 'string' && url.includes('bricklink.com'));
  ...
  const single =
    ...
    (typeof payload.rebrickable_part_num === 'string' &&
      payload.rebrickable_part_num) ||
    (typeof payload.bricklink_part_num === 'string' &&
      payload.bricklink_part_num) ||
    (typeof payload.bricklink_part_num === 'number' &&
      String(payload.bricklink_part_num)) ||
    '';
  if (single) {
    out.push({
      partNum: single,
