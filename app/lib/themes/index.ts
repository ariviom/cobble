...
function createThemeResolvers(themeById: Map<number, ThemeLike>) {
	...
	const getThemePath = (themeId: ThemeId): string | null => {
		...
		while (current && !visited.has(current.id)) {
			...
			current = current.parent_id != null ? themeById.get(current.parent_id) : undefined;
		}
	...
	const getThemeMeta = (themeId: ThemeId): { themeName: string | null; themePath: string | null } => {
		const theme = Number.isFinite(themeId as number) ? themeById.get(themeId as number) : undefined;
		return {
