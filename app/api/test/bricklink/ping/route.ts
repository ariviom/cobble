...
export async function GET(req: NextRequest) {
  ...
  const colorId =
    colorIdParam && colorIdParam.trim() !== ''
      ? Number(colorIdParam)
      : undefined;
  ...
  try {
    ...
    result.subsetsSample =
      Array.isArray(subsets) && subsets.length ? subsets[0] : null;
  } catch (e) {
  ...
  try {
    ...
    result.supersetsSample =
      Array.isArray(supersets) && supersets.length ? supersets[0] : null;
  } catch (e) {
  ...
  if (typeof effectiveColorId === 'number') {
    try {
      ...
      result.supersetsByColorCount = Array.isArray(supByColor)
        ? supByColor.length
        : 0;
